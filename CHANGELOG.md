...
- 同步 重构结束阅读收尾：阅读时间与最终精确进度独立处理，最终位置固定后以 `rt=0` 提交并回读确认；返回主页不再等待串行上传，旧待确认任务不会覆盖更新的位置。
- Kindle 修复熄屏后台任务可能自行退出屏保的问题，区分电源键、翻盖与系统内部休眠事件；锁屏下载期间保持网络与任务运行，完成后再进入真正休眠。
- Kobo 在原生休眠关闭 Wi-Fi 前接管有活动下载的锁屏流程，避免熄屏后下载中断；重复 Suspend 不再反复关闭 Wi-Fi，下载完成后恢复 Kobo 原生安全休眠。
- 兼容 Android 与未知设备不启用 Kindle/Kobo 专用休眠能力；真实休眠时保存下载断点，开屏后继续恢复，避免特殊后台逻辑影响其他设备。
...
- 本地书 普通本地书在 KOReader 确认打开后固定走本地阅读路径，不再进入微信读书进度、阅读时间、批注同步或身份识别；当前不支持的格式不会提前冻结主页或切换锁屏封面。
- 阅读 本地书返回主页改为直接关闭 KOReader Reader，删除未知本地文件的整文件身份扫描；锁屏封面和最近阅读只在 Reader Ready 后更新，并增加打开失败自动恢复。
- 界面 本地书隐藏微信评论、同步、下载生成与修复入口，快捷栏以“目录”替代“评论”；微信读书书籍继续保留原有同步与评论功能。
- 更新 OTA 改为新版本启动试运行后再确认，首次启动未完成时保留旧版本备份；若新版本未能正常启动，下次启动可自动恢复上一版本。
- 设备 Bluetooth 能力检测移出启动关键路径，并为外部蓝牙命令增加超时保护，系统蓝牙服务异常时不再阻塞 KOReader。
- 稳定 设置文件改为完整生成并校验后再替换，避免 `miuread.lua` 在退出阅读或状态切换时留下半写入文件。
...
- 性能 重构大书架主页与后台任务调度，只优先处理当前可见页并复用已生成封面，降低封面、进度、扫描和下载并发造成的卡顿。
- 下载 强化低内存、断网、锁屏与唤醒恢复，图片章节改为更稳妥的流式处理；健康下载可在熄屏后继续，异常任务保留断点恢复。
- 同步 分离阅读时间、阅读中进度与结束阅读批注同步；阅读进度优先使用微信读书原生精确坐标，坐标未准备好时只上传时间，避免粗略进度覆盖准确位置。
- 稳定 完善 Reader 返回、休眠收尾、最近阅读与锁屏封面逻辑；觅阅封面不可用时回退 KOReader 原生锁屏，减少主页冻结画面、旧任务抢占和重复收尾。
