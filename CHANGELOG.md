...
- Kindle 修复锁屏后台下载完成后可能无法正常唤醒：自动休眠等待真实休眠确认后才释放保护，电源键与翻盖操作优先，交接失败时恢复到可正常操作状态。
- Kobo 修复后台下载结束与原生休眠、翻盖/电源键可能互相竞争的问题；下载完成后保持保护直到原生休眠接管，用户主动唤醒可取消尚未完成的自动休眠。
- 兼容 保留 Kindle/Kobo 熄屏继续下载；Android 与未知设备仍不启用专用电源控制，下载、断点续传、阅读进度、阅读时间与批注同步逻辑保持不变。
