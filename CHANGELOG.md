...
- 进度目录语义：修正单章/章节版把“本地已选章节数”写成“整书目录章节数”的问题；新记录明确保存本地章节数、整书目录数和完整目录标记，旧 beta.14～17 下载可通过已保存的整书目录哈希自动恢复。
- 精确进度显示：`chapterUid + chapterOffset` 继续作为精确坐标；精确正文锚点得到的章内比例和整书映射不再被 KOReader 的整本页数比例覆盖，inverse/page ratio 只保留为估算诊断。
- 阅读工具栏：正常翻页不再调用页数映射冒充“全书进度”；阅读稳定后仅使用本地坐标正文缓存异步刷新精确进度，连续翻页自动合并，只计算最后停留位置。
- 显示可信度：精确正文结果显示两位小数；只有分页估算时明确显示 `≈`，单章/章节版缺完整目录时显示“全书待换算”，不再出现伪精确百分比。
- 上传确认：微信读书已经切到目标章节但暂时回读 `chapterOffset=0` 时视为云端坐标传播中，延迟复核并保留 pending，不再立即重复提交同一个精确位置。
- 性能边界：打开“阅读进度”不再同步扫描整章或构建 PosMap；显示定位只走后台子进程和本地缓存，不因显示百分比主动联网；beta.17 的连续阅读、下一章预下载、首页统计和锁屏逻辑保持不变。
