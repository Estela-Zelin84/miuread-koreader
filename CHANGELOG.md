...
- 稳定 图片章节改为磁盘流式下载与逐项处理，长任务持续无有效进展时会安全停止并从断点恢复一次，降低低内存设备下载假死。
- 性能 新增统一后台任务与内存压力调度，主页、阅读、封面、扫描、同步和下载互相避让，封面按当前页渐进补全，减少老设备卡顿。
- 休眠 区分锁屏下载保活与真正休眠，完善唤醒恢复、阅读时间保存和 Reader 内部重载判断，减少休眠后下载停滞与异常跳回主页。
- 修复 兼容连续多段正文并修复残缺章节，完善脚注高亮与批注同步、Bluetooth、KOReader 一级入口、锁屏封面及原生阅读设置。
