...
function MemoryMode:set_enabled(enabled)
    ...
    if enabled then
        ...
        logger.info("[MiuRead][MemoryMode] enabled", "ratio=", tostring(TARGET_RATIO))
    ...
    logger.info("[MiuRead][MemoryMode] disabled", "external_change=", tostring(external_change))
...
function MemoryMode:restore_detected()
    ...
    logger.info("[MiuRead][MemoryMode] detected target restored",
